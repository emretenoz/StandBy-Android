...
private val defaultLeftWidgets = listOf(StandByWidgetType.CLOCK, StandByWidgetType.BATTERY)
private val defaultRightWidgets = listOf(StandByWidgetType.DATE, StandByWidgetType.CHARGING)

internal fun decodeWidgets(
    encoded: String?,
...
internal fun toggleWidgetInList(
    widgets: List<StandByWidgetType>,
    widget: StandByWidgetType,
): List<StandByWidgetType> = when {
    widget !in widgets -> widgets + widget
    widgets.size > 1 -> widgets - widget
    else -> widgets
}

internal fun moveWidgetInList(
    widgets: List<StandByWidgetType>,
    fromIndex: Int,
    toIndex: Int,
): List<StandByWidgetType> {
    if (fromIndex !in widgets.indices || toIndex !in widgets.indices) return widgets
...
class SettingsRepository(private val context: Context) {
    private object Keys {
        ...
        val worldClockZone = stringPreferencesKey("world_clock_zone")
        val weatherEnabled = booleanPreferencesKey("weather_enabled")
    }

    val settings: Flow<StandBySettings> = context.settingsDataStore.data
        .catch { error ->
        ...
        .map { preferences ->
            StandBySettings(
                ...
                batteryWidgetStyle = enumValue(
                    preferences[Keys.batteryWidgetStyle],
                    BatteryWidgetStyle.CIRCULAR,
                ),
                worldClockZone = enumValue(preferences[Keys.worldClockZone], WorldClockZone.UTC),
                weatherEnabled = preferences[Keys.weatherEnabled] ?: false,
                leftWidgets = decodeWidgets(preferences[Keys.leftWidgets], defaultLeftWidgets),
                rightWidgets = decodeWidgets(preferences[Keys.rightWidgets], defaultRightWidgets),
            )
        }
    ...
    suspend fun setWorldClockZone(zone: WorldClockZone) = setEnum(Keys.worldClockZone, zone)
    suspend fun setWeatherEnabled(enabled: Boolean) = update(Keys.weatherEnabled, enabled)
    ...
    suspend fun toggleWidget(column: WidgetColumn, widget: StandByWidgetType) {
        ...
        val defaults = if (column == WidgetColumn.LEFT) defaultLeftWidgets else defaultRightWidgets
        context.settingsDataStore.edit { preferences ->
            val widgets = toggleWidgetInList(decodeWidgets(preferences[key], defaults), widget)
    ...
    suspend fun moveWidget(column: WidgetColumn, fromIndex: Int, toIndex: Int) {
        ...
        val defaults = if (column == WidgetColumn.LEFT) defaultLeftWidgets else defaultRightWidgets
        context.settingsDataStore.edit { preferences ->
            val widgets = moveWidgetInList(
                decodeWidgets(preferences[key], defaults),
                fromIndex,
                toIndex,
            )
            preferences[key] = widgets.joinToString(",") { it.name }
